...
ALTER TABLE `sop`.`t_pcs_goodslog`   
  ADD COLUMN `afUpNum` DECIMAL(12,3) NULL AFTER `tempDeliverNum`,
  ADD COLUMN `afDiffNum` DECIMAL(12,3) NULL AFTER `afUpNum`;
...
BEGIN
	...
	ELSE   
		...
		IF g_flag=1 THEN 
			SELECT CASE WHEN i.afUpNum IS NOT NULL AND i.afUpNum!=0 THEN i.afUpNum ELSE(wb.actualRoughWeight-wb.actualTareWeight)END ,CASE WHEN i.afUpNum IS NOT NULL AND i.afUpNum!=0 THEN i.afUpNum-i.actualNum ELSE (wb.actualRoughWeight-wb.actualTareWeight-i.actualNum) END INTO g_afUpNum,g_afUpDiffNum FROM t_pcs_weighbridge wb,t_pcs_goodslog i,t_pcs_train t WHERE t.id=l_batchId AND i.batchId=t.id AND i.deliverType=1 AND i.serial=wb.serial;
		ELSE 
			SELECT CASE WHEN i.afUpNum IS NOT NULL AND i.afUpNum!=0 THEN i.afUpNum ELSE (m.actualMeterEnd-m.actualDeliverNum) END ,CASE WHEN i.afUpNum IS NOT NULL AND i.afUpNum!=0 THEN i.afUpNum-i.actualNum ELSE(m.actualMeterEnd-m.actualDeliverNum-i.actualNum) END INTO g_afUpNum,g_afUpDiffNum FROM t_pcs_measure m,t_pcs_train t,t_pcs_goodslog i WHERE t.id = m.trainId AND t.id = l_batchId AND i.batchId=t.id AND i.deliverType=1;
		END IF;
		SELECT 	  t.isChange,i.id,	  i.batchId,	  i.goodsId,	  i.deliverNum,	  c.name ladingClientName, IFNULL((SELECT l.code FROM t_pcs_lading l
	  WHERE l.id = (SELECT ladingId FROM t_pcs_goods_group WHERE id = g.goodsGroupId)),'') ladingCode,p.name productName,
	    i.deliverNum,g.code goodsCode,(g.goodsCurrent-(g.goodsTotal-(CASE WHEN g.goodsInPass-g.goodsOutPass>=0 THEN g.goodsOutPass ELSE g.goodsInPass END))) goodsCurrent,
	    i.actualNum,IFNULL(g_afUpNum,0) g_afUpNum,IFNULL(g_afUpDiffNum,0) g_afUpDiffNum FROM t_pcs_goodslog i LEFT JOIN t_pcs_client c ON c.id = i.ladingClientId LEFT JOIN t_pcs_goods g ON g.id=i.goodsId LEFT JOIN t_pcs_train t ON t.id=i.batchId,
	  t_pcs_product p WHERE i.deliverType = l_type  AND i.batchId = l_batchId AND g.id = i.goodsId AND p.id=g.productId AND i.type=5;
	 END IF;
