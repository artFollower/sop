 ...
 */+ function() {"use strict";
 ...
 var Grid = function(element, options) {
		...
		this.showPage = options.showPage;//是否分页
		this.isPrint = options.isPrint;//是否打印
		this.printHeadder = options.printHeadder;//打印页眉
		this.printFotter = options.printFotter;//打印页脚
		this.callback = options.callback;//回掉函数
		...
		this.stateSave=options.stateSave;
	...
	Grid.prototype = {
		...
		_initLayout : function() {
			...
			this.print = this.$element.find('.btn-print');
			var width = this.$element.css("width");
			this.gridBody = this.$element.find('.grid-body').css('width', width);
//			this.gridBody = this.$element.find('.grid-body').css('width', '100%');
//			this.gridTableHead = this.$element.find('.grid-table-head').css('min-width', this.$element.width());
			this.gridTableHead = this.$element.find('.grid-table-head').css('min-width', '50%');
			this.gridTableHeadTable = this.gridTableHead;//.find('table');
			var width = this.$element.width();
			
//			this.gridBody.css('width', width);
			this.gridBody.css('width', '100%');
//			this.gridTableBody = this.$element.find('.grid-table-body').css('width', width);
			this.gridTableBody = this.$element.find('.grid-table-body').css('width', '100%');
			this.gridTableBody = this.$element.find('.grid-table-body').css('height', 'auto');
...
//					titleHtml.push('<th index="' + i + '" width="'+column.width+'px"');
//				}else {
//					titleHtml.push('<th index="' + i + '"');
//				}
				...
				titleHtml.push('style="text-align:center;">');
//				titleHtml.push(' style="border: 1px solid #ddd;">');
				titleHtml.push(column.title);
				if (!this.options.lockWidth) {
					titleHtml.push('<div class="colResize"></div>');
//					titleHtml.push('<div class="colResize" style="cursor: col-resize;float: right;height: 20px;left: 8px;position: relative;width: 8px;"></div>');
			...
			if (totalColumnWidth > this.$element.width()) {
				...
				self.$element.find('.grid-body').css('width', totalColumnWidth);
			} else {
				this.gridTableHead.css('width', this.$element.width());
//				this.gridTableHead.css('width', "100%");
//				this.gridTableBodyTable.css('width', "100%");
//				self.$element.find('.grid-body').css('width', "100%");
		...
		_initEvents : function() {
			...
			if(this.isPrint) {
				...
				this.print.click(function() {
					CommonPrint.gridPrint(self,self.printHeadder,self.printFotter);
				});
			}
		},
		...
		_loadData : function() {
			...
			this.gridTableBody.append("<div id='loading-box' style='margin:auto;width:100%;height:100%;z-index:39999;box-shadow:4px 4px 3px rgba(20%,20%,40%,0.5);position: absolute;cursor: not-allowed;'>"+
...
//			console.log(self.searchCondition);
//			console.log(self);
//			console.log(self.stateSave);
//			console.log(self.gridName);
//			console.log(currentGrid);
//			console.log(currentSearchCondition);
			
			
			var isFirst = sessionStorage.getItem("repairType");  
			//如果是从其他页面过来的
			if(isFirst==0){
				
				sessionStorage.setItem("repairType", 1); 
				
				//如果其他页面跳转到前一个页面
				if(currentGrid==self.gridName&&self.gridName){
					self.searchCondition=currentSearchCondition;
					
					params.page=currentPageNo;
					params.pagesize = currentPageSize;
					self.pageSize= currentPageSize;
					self.pageNo=currentPageNo;
				for (var prop in currentSearchCondition) {
					if($("[name='"+prop+"']").length!=0){ 
						
						switch ($("[name='"+prop+"']")[0].tagName) {
						case 'INPUT':
							if(typeof($("[name='"+prop+"']").attr("data"))!="undefined"||$("[name='"+prop+"']").attr("data-provide")=="typeahead"){
								$("[name='"+prop+"']").attr("data",currentSearchCondition[prop]);
								$("[name='"+prop+"']").val(currentSearchName[prop]);
								
							}else{
								if(currentSearchCondition[prop]!='-1'){
									if((prop=="startTime"||prop=="endTime")&&!isNaN(currentSearchCondition[prop])){
										...
										$("[name='"+prop+"']").val(new Date(currentSearchCondition[prop]*1000).format("yyyy-MM-dd"));
									}else{
										
										$("[name='"+prop+"']").val(currentSearchCondition[prop]);
									}
								}
							...
							break;
						case 'SELECT':
							$("[name='"+prop+"']").val(currentSearchCondition[prop]).trigger("change"); 
				...
				for (var prop in currentSearchCheck) {
					if(currentSearchCheck[prop]==1){
						$("[id='"+prop+"']").attr("checked",true);
					}
				}
//				if(currentTab){
////					$("#"+currentTab).click();
//					
//					if(self.tabCallBack != null && typeof self.tabCallBack === "function") {
//						self.tabCallBack(currentTab);
//					}
//					
//				}
				
				
				if(self.backCallBack != null && typeof self.backCallBack === "function") {
					self.backCallBack(currentSearchCondition);
				}
			...
			if(self.stateSave){
				...
				currentPageNo=0;
				currentPageSize=0;
				currentGrid=self.gridName;
				currentSearchCondition=self.searchCondition;
				currentPageNo=params.page;
				currentPageSize=params.pagesize;
				for (var prop in self.searchCondition) {
//					$("[name="+prop+"]").val(self.searchCondition[prop]);
//					params[prop] = self.searchCondition[prop];
//					console.log($("[name='"+prop+"']")[0].tagName);
					
					if($("[name='"+prop+"']").length!=0){ 
					switch ($("[name='"+prop+"']")[0].tagName) {
					case 'INPUT':
						if(typeof($("[name='"+prop+"']").attr("data"))!="undefined"){
							currentSearchName[prop]=$("[name='"+prop+"']").val();
						}
						
						
						if($("[id='"+prop+"']").attr("type")=="checkbox"){
							if($("[id='"+prop+"']").is(':checked')){
								currentSearchCheck[prop]=1;
							}
						}
...
//					else{
//						self.searchCondition.remove(prop);
//					}
				}
...
//			currentGrid=
			...
			if (self.options.isUserLocalData) {
				...
				if (!self.options.localData || self.options.localData.length == 0) {
					...
					self.gridTableBody.append($('<tr><td colspan="30"><div data-role="noData" style="font-size:16px ; padding: 20px; width:' + self.gridTableBodyTable.width() + 'px;">' + self.options.noDataText + '</div></td></tr>'));
...
//				dataType : 'json',
				success : function(result){
					...
					if (!result.data) {
						...
						self.gridTableBody.loader('hide');
						setTimeout(function() {
							window.location.href = config.getDomain()+"/login";
						},1000);
						return;
					}
					var start = self.pageSize * self.pageNo;
					if(result.data.length > 0) {
						...
						self.startRecord.text(0);
					}
					self.endRecord.text(start + result.data.length);
					self.totalRecordHtml.text(result.map.totalRecord);
					//self._initPageNo(result.Total)
					self.items = result.data;
					self.totalRecord = result.map.totalRecord;
					if (result.data.length == 0) {
						self.pages.hide();
						self.gridTableBodyTable.empty();
						self.gridTableBody.find('[data-role="noData"]').remove();
						self.gridTableBody.append($('<tr><td colspan="30"><div data-role="noData" style="font-size:16px ; padding: 20px; width:100%;">' + self.options.noDataText + '</div></td></tr>'));
						self.gridTableBody.loader('hide');
						self.$element.trigger('complate');
					} else {
						self.pages.show();
...
//					if(self.$element.find('.grid-body').width() > self.$element.width()) {
//						self.$element.find('.grid-body').css('width', self.$element.width());
//					}
					if(self.$element.find('.grid-body').width() ==0) {
						self.$element.find('.grid-body').css('width', '100%');
					}
					self.$element.find('.grid-body').css('overflow-y', 'auto');
					if(self.callback != null && typeof self.callback === "function") {
						self.callback();
					}
					try{
						...
						console.log(1);
						$('#scrollbar').children(".viewport").css("height",$('#scrollbar').find(".overview").height()+"px");
						//$('#scrollbar').children(".viewport").css("width",$('#scrollbar').find(".grid-table-body").width()+"px");
					}catch(err) {}
					/*try{
						self.$element.find(".grid-body").children("table").freezeHeader();
					}catch(err) {}*/
				},
		...
		_initPageNo : function() {
			...
			pageHtml.push('<li data-role="firstPage"><a href="javascript:void(0);">&laquo;</a></li>');
			pageHtml.push('<li data-role="prev"><a href="javascript:void(0);">&lsaquo;</a></li>');
			if (self.pageNo % self.showPage == 0) {
				self.pageNo != Grid.DEFAULTS.pageNo && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
				for (var i = self.pageNo; i < self.totalPage && i < (self.pageNo + self.showPage); i++) {
					pageHtml.push('<li data-value="' + i + '" data-role="pageNo"><a href="javascript:void(0);">' + (i + 1) + '</a></li>');
				}
				(self.pageNo + self.showPage) < self.totalPage && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
			} else {
				var start = Math.floor(self.pageNo / self.showPage) * self.showPage;
				start != Grid.DEFAULTS.pageNo && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
				for (var i = start; i < self.totalPage && i < (start + self.showPage); i++) {
					pageHtml.push('<li data-value="' + i + '" data-role="pageNo"><a href="javascript:void(0);">' + (i + 1) + '</a></li>');
				}
				(start + self.showPage) < self.totalPage && pageHtml.push('<li><a href="javascript:void(0);">...</a></li>');
			}
			pageHtml.push('<li data-role="next"><a href="javascript:void(0);">&rsaquo;</a></li>');
			pageHtml.push('<li data-role="lastPage" ><a href="javascript:void(0);">&raquo;</a></li>');
...
//						trHtml.push('<td index="' + k + '" width="' + width + '"');
		 ...
		 */
		destory : function() {
			this.$element.data('skycloud.grid', null);
			this.$element.empty();
		},
		/**
		 * 外部查询
	...
	$.fn.getGrid = function() {
		return $(this).data('skycloud.grid');
	};
//	Grid.DEFAULTS.TEMPLATE = '<div class="table-responsive"><table class="table table-responsive table-bordered grid"><thead><tr><th><div class="btn-group buttons"></div><div class="search"><div class="btn-group select " data-role="condition"></div><div class="input-group" style="width:180px;"><input type="text" class="input-medium form-control" placeholder="Search" data-role="searchValue"><div class="input-group-btn"><button type="button" class="btn btn-default" data-role="searchBtn"><span class="glyphicon glyphicon-search"></span></button></div></div></div></th></tr></thead><tbody><tr><td><div class="colResizePointer"></div><div class="grid-body"><div class="grid-table-head"><table class="table table-bordered"></table></div><div class="grid-table-body"><table class="table table-responsive table-bordered table-hover table-striped"></table></div></div></td></tr></tbody><tfoot><tr><td><div class="records">显示:<span data-role="start-record">1</span>-<span data-role="end-record">10</span>, 共<span data-role="total-record">0</span>条记录。&nbsp;每页显示:<div class="btn-group select " data-role="pageSizeSelect"></div>条</div><div><div class="btn-group pages"><ul class="pagination"></ul></div></div></td></tr></tfoot></table></div>';
	Grid.DEFAULTS.TEMPLATE = '<div class="table-responsive" style="overflow-x: auto;"><table class="table table-responsive table-bordered grid"><thead><tr><th><div class="btn-group buttons"></div><div class="search"><div class="btn-group select " data-role="condition"></div><div class="input-group" style="width:180px;"><input type="text" class="input-medium form-control" placeholder="Search" data-role="searchValue"><div class="input-group-btn"><button type="button" class="btn btn-default" data-role="searchBtn"><span class="glyphicon glyphicon-search"></span></button></div></div></div></th></tr></thead><tbody><tr><td><div class="colResizePointer"></div><div class="grid-body" style="min-height: 150px;"><table class="table table-responsive table-bordered table-hover" style="margin: 0px; border: 0px;"><thead class="grid-table-head"></thead><tbody class="grid-table-body"></tbody></table></div></td></tr></tbody><tfoot><tr><td><div class="records">显示:<span data-role="start-record">0</span>-<span data-role="end-record">10</span>, 共<span data-role="total-record">0</span>条记录。&nbsp;每页显示:<div class="btn-group dropup select2-container" data-role="pageSizeSelect"></div>条</div><div><button type="button" class="btn btn-default mar-r-10 btn-print hidden"><span class="fa fa-print"></span><span class="text">打印预览</span></button><div class="btn-group pages"><ul class="pagination"></ul></div></div></td></tr></tfoot></table></div>';
	var old = $.fn.grid;
	$.fn.grid = function(option) {
		return this.each(function() {
			var $this = $(this);
			var data = $this.data('skycloud.grid');
			var options = $.extend({}, Grid.DEFAULTS, $this.data(), typeof option == 'object' && option);
			if (!data) {
				$this.data('skycloud.grid', ( data = new Grid(this, options)));
			}
 ...
 */ + function($) {"use strict";
	...
	ModifyPassword.prototype.save = function(thiz) {
		...
		var url = contextPath + '/auth/currentUser/updatePassword.skycloud';
		$.ajax({
			type : "post",
			url : url,
			data : data,
			...
			success:function(msg){
				if (msg.success) {
					...
					var logOut = contextPath+"/logout.skycloud";
					$.post(logOut,function(data){
						if(data.success){
							window.location.href = contextPath+"/login.skycloud";
						}
					});
				} else {
					self.$element.find('.modal-content').message({
	...
	$.fn.modifyPassword = function(option) {
		...
		return this.each(function() {
			var $this = $(this);
			var data = $this.data('skycloud.modifyPassowrd');
			var options = $.extend({}, ModifyPassword.DEFAULTS, $this.data(), typeof option == 'object' && option)
			$this.data('skycloud.modifyPassowrd', ( data = new ModifyPassword(this, options)));
		});
	};
 ...
 */ + function($) {"use strict" ;
	...
	Select.DEFAULTS.TEMPLATE = '<button type="button" class="btn btn-default" style="margin-right: 0px;" data-toggle="item">' + '</button><button type="button" class="btn btn-default dropdown-toggle" data-toggle="button">' + '<span class="caret" style="height:14px;margin-top:6px;"></span>' + '</button>' + '<input type="hidden" data-toggle="value"/>' + '<ul class="dropdown-menu" role="menu"></ul>';
	$.fn.getValue = function() {
		if ($(this).data('skycloud.select')) {
			return $(this).data('skycloud.select').getValue();
		}
	};
	$.fn.getItem = function() {
		if ($(this).data('skycloud.select')) {
			return $(this).data('skycloud.select').getItem();
		}
	};
	$.fn.setValue = function(value) {
		if ($(this).data('skycloud.select')) {
			return $(this).data('skycloud.select').selectByValue(value);
		}
	};
	$.fn.setSelectItems = function(contents) {
		if ($(this).data('skycloud.select')) {
			return $(this).data('skycloud.select').setItems(contents);
		}
	};
	...
	$.fn.resetItems = function(contents) {
		$(this).data('skycloud.select').$item.empty();
		return $(this).data('skycloud.select').setItems(contents);
	};
	var old = $.fn.select;
	$.fn.select = function(option) {
		return this.each(function() {
			var $this = $(this);
			//var data = $this.data('skycloud.select');
			var data;
			var options = $.extend({}, Select.DEFAULTS, $this.data(), typeof option == 'object' && option);
			//if (!data) {
				$this.data('skycloud.select', ( data = new Select(this, options)));
			//}
			if ( typeof option == 'string') {
				data[option]();
			}
 ...
 */ + function($) {"use strict";
	var Confirm = function(container, options) {
		...
		if(options.concel) {
			this.$element = $(Confirm.DEFAULTS.NOCONCELTEMPLATE);
		}
		this.options = options;
		this.init();
	};
	...
	Confirm.prototype.init = function() {
		...
		this.$element.find('[data-role="cancelBtn"]').on('click',function(){
 ...
 */ + function($) {"use strict";
	...
	Loader.prototype.hide = function() {
		...
		self.$element.data('skycloud.loader', null);
	}
	var old = $.fn.loader;
	$.fn.loader = function(option) {
		return this.each(function() {
			var $this = $(this);
			var data = $this.data('skycloud.loader');
			var options = $.extend({}, Loader.DEFAULTS, $this.data(), typeof option == 'object' && option);
			if (!data) {
				$this.data('skycloud.loader', ( data = new Loader(this, options)));
			}
			if ( typeof option == 'string') {
				data[option]();
			}
