...
Date.jab2date = function(ts) {
	...
	if (ts.substr(ts.length - 6, 1) != 'Z') {
		...
		if (ts.substr(ts.length - 6, 1) == '+') date.setTime(date.getTime()
				- offset.getTime());
...
XmlHttp.create = function() {
	try {
		...
		if (window.ActiveXObject) { return new ActiveXObject(XmlHttp
				.getPrefix()
				+ ".XmlHttp"); }
	}
	catch (ex) {
...
XmlDocument.create = function(name, ns) {
	...
	try {
		...
		if (!doc.documentElement
			...
			try {
				if (ns != '') doc.appendChild(doc.createElement(name))
...
var JSJaCBuilder = {
	buildNode : function(doc, elementName) {
		var element;
		if (arguments[2]) if (JSJaCBuilder._isStringOrNumber(arguments[2])
			...
			if (arguments[2]['xmlns']) {
				try {
					element = doc.createElementNS(arguments[2]['xmlns'],
							elementName);
				}
				catch (e) {
					element = doc.createElement(elementName);
			...
			else element = doc.createElement(elementName);
		...
		if (arguments[3]) JSJaCBuilder._children(doc, element, arguments[3]);
		return element;
	},
...
JSJaCConnection.prototype.registerHandler = function(event) {
	...
	this._events[event] = this._events[event].sort(function(a, b) {
		with (a) {
			...
			if (childNS == '*') {
				if (b.childNS == '*') return 0;
				return 1;
			}
			if (childName == '*') {
				if (b.childName == '*') return 0;
				return 1;
			}
		}
		return -1;
...
JSJaCConnection.prototype._doLegacyAuth2 = function(iq) {
	...
	query = iq.appendNode('query', {
		xmlns : 'jabber:iq:auth'
	}, [ [ 'username', oCon.username ], [ 'resource', oCon.resource ] ]);
	if (use_digest) {
		query.appendChild(iq.buildNode('digest', hex_sha1(oCon.streamid
				+ oCon.pass)));
	}
	else if (oCon.allow_plain) {
...
JSJaCConnection.prototype._doSASLAuthDigestMd5S2 = function(req) {
	...
	if (doc.firstChild.nodeName == 'failure') {
		if (doc.firstChild.xml) this.oDbg.log("auth error: "
				+ doc.firstChild.xml, 1);
	...
	if (doc.firstChild.nodeName == 'success') this._reInitStream(this.domain,
			'_doStreamBind');
	else this._sendRaw("<response xmlns='urn:ietf:params:xml:ns:xmpp-sasl'/>",
			'_doSASLAuthDone');
};
JSJaCConnection.prototype._doSASLAuthDone = function(req) {
	...
	else this._reInitStream(this.domain, '_doStreamBind');
};
...
JSJaCConnection.prototype._process = function(timerval) {
	...
	if (timerval) this.setPollInterval(timerval);
	...
	this._req[slot].r.onreadystatechange = function() {
		...
		if (oCon._req[slot].r.readyState == 4) {
			...
			if (oCon._pQueue.length) {
				...
				oCon.oDbg.log("scheduling next poll in "
						+ oCon.getPollInterval() + " msec", 4);
				oCon._timeout = setTimeout("oCon._process()", oCon
						.getPollInterval());
			}
		}
	};
...
JSJaCConnection.prototype._sendEmpty = function JSJaCSendEmpty() {
	...
	this._req[slot] = this._setupRequest(true);
	this._req[slot].r.onreadystatechange = function() {
		if (typeof (oCon) == 'undefined' || !oCon) return;
		if (oCon._req[slot].r.readyState == 4) {
			oCon.oDbg.log("async recv: " + oCon._req[slot].r.responseText, 4);
			oCon._getStreamID(slot);
		}
	}
	if (typeof (this._req[slot].r.onerror) != 'undefined') {
		this._req[slot].r.onerror = function(e) {
...
function JSJaCHttpBindingConnection(oArg) {
	...
	this._getFreeSlot = function() {
		for (var i = 0; i < this._hold + 1; i++)
			if (typeof (this._req[i]) == 'undefined'
					...
					|| this._req[i].r.readyState == 4) return i;
		return -1;
	}
	...
	this._suspend = function() {
		...
		reqstr += "</body>";
		var abortTimerID = setTimeout("oCon._req[" + slot + "].r.abort();",
				5000);
...
//	this._httpbase="/mSop/JHB";
	...
	if (oArg.secure) reqstr += " secure='" + this.secure + "'";
	if (JSJAC_HAVEKEYS) {
		this._keys = new JSJaCKeys(hex_sha1, this.oDbg);
	...
	this._req[slot].r.onreadystatechange = function() {
		...
		if (oCon._req[slot].r.readyState == 4) {
			oCon.oDbg.log("async recv: " + oCon._req[slot].r.responseText, 4);
			oCon._handleInitialResponse(slot);
		}
	}
	if (typeof (this._req[slot].r.onerror) != 'undefined') {
		this._req[slot].r.onerror = function(e) {
...
function JSJaCHBCDisconnect() {
	...
	var slot = this._getFreeSlot();
	this._req[slot] = this._setupRequest(false);
	var reqstr = "<body type='terminate' xmlns='http://jabber.org/protocol/httpbind' sid='"
			+ this._sid + "' rid='" + this._rid + "'";
	if (JSJAC_HAVEKEYS) {
		reqstr += " key='" + this._keys.getKey() + "'";
	}
...
function JSJaCHBCGetRequestString(raw) {
	...
	if (this._rid <= this._last_rid
			&& typeof (this._last_requests[this._rid]) != 'undefined') reqstr = this._last_requests[this._rid].xml;
	else {
		var xml = '';
		while (this._pQueue.length) {
			var curNode = this._pQueue[0];
...
function JSJaCHttpPollingConnection(oArg) {
	...
	this._getFreeSlot = function() {
		if (typeof (this._req[0]) == 'undefined'
				...
				|| this._req[0].r.readyState == 4) return 0;
		else return -1;
	}
...
function JSJaCHPCPrepareResponse(r) {
	...
	try {
		var doc = JSJaCHttpPollingConnection.parseTree("<body>"
				+ req.responseText + "</body>");
		if (!doc || doc.tagName == 'parsererror') {
			this.oDbg.log("parsererror", 1);
			doc = JSJaCHttpPollingConnection
					.parseTree("<stream:stream xmlns:stream='http://etherx.jabber.org/streams'>"
							+ req.responseText);
			if (doc && doc.tagName != 'parsererror') {
...
function JSJaCHPCDisconnect() {
	...
	if (this._timeout) clearTimeout(this._timeout);
	this._req[0] = this._setupRequest(false);
	if (JSJAC_HAVEKEYS) this._req[0].r.send(this._sid + ";"
			+ this._keys.getKey() + ",</stream:stream>");
	else this._req[0].r.send(this._sid + ",</stream:stream>");
	try {
		JSJaCCookie.read('JSJaC_State').erase();
	}
	catch (e) {
...
JSJaCJID._checkNodeName = function(nodeprep) {
	...
	for (var i = 0; i < JSJACJID_FORBIDDEN.length; i++) {
		if (nodeprep.indexOf(JSJACJID_FORBIDDEN[i]) != -1) { throw new JSJaCJIDInvalidException(
				"forbidden char in nodename: " + JSJACJID_FORBIDDEN[i]); }
	}
};
...
function JSJaCPacket(name) {
	this.name = name;
	if (typeof (JSJACPACKET_USE_XMLNS) != 'undefined' && JSJACPACKET_USE_XMLNS) this.doc = XmlDocument
			.create(name, 'jabber:client');
...
JSJaCPacket.prototype.setFrom = function(from) {
	...
	else if (typeof (from) == 'string') this.getNode().setAttribute('from',
			from);
...
JSJaCPacket.prototype.getChild = function(name, ns) {
	...
	for (var i = 0; i < nodes.length; i++) {
		...
		if (name != '*' && nodes.item(i).tagName != name) continue;
...
JSJaCPacket.prototype.xml = function() {
	if (this.getDoc().xml) return this.getDoc().xml;
...
JSJaCPacket.prototype._replaceNode = function(aNode) {
	...
	for (var i = 0; i < aNode.childNodes.length; i++)
		if (this.getDoc().importNode) this.getNode().appendChild(
				this.getDoc().importNode(aNode.childNodes.item(i), true));
		else this.getNode().appendChild(
				aNode.childNodes.item(i).cloneNode(true));
};
JSJaCPacket.prototype._setChildNode = function(nodeName, nodeValue) {
	...
	if (aNode) try {
		...
		aNode = this.getNode().appendChild(
				this.getDoc().createElement(nodeName));
...
JSJaCPacket.prototype.appendNode = function(element) {
	if (typeof element == 'object') {
		...
		return this.getNode().appendChild(
				this.buildNode(element, arguments[1], arguments[2]));
	}
};
...
JSJaCIQ.prototype.reply = function(payload) {
	...
	rIQ.setType('result');
	if (payload) {
		if (typeof payload == 'string') rIQ.getChild.appendChild(rIQ.getDoc()
				.loadXML(payload));
		else if (payload.constructor == Array) {
			var node = rIQ.getChild();
			for (var i = 0; i < payload.length; i++)
				if (typeof payload[i] == 'string') node.appendChild(rIQ
						.getDoc().loadXML(payload[i]));
