var CargoPK = function() {
	...
	var handleRecords = function() {
		...
		}, 
		{
			...
			name : "cargodata",
			render: function(item, name, index){
				var re="<a href='javascript:void(0)' onclick='CargoPK.editCargo("+index+")' class='btn btn-xs blue'><span class='glyphicon glyphicon glyphicon-edit' title='修改'></span></a>";
...
//				
		...
		$('[data-role="virtualGrid"]').grid({
			...
			isShowPages : true,
			url : config.getDomain()+"/storageConfirm/list?cargoStatus=4"
		});
...
//		$('#ajax').modal('show');
		$.get(config.getResource()+"/pages/inbound/cargoPK/addCustomer.jsp").done(function(data){
			dialog = $(data) ;
			initDialog(dialog);
...
//						 dialog.remove();
	...
	function editCargo(index){
		$.get(config.getResource()+"/pages/inbound/cargoPK/editCustomer.jsp").done(function(data){
			dialog = $(data) ;
			initDialog(dialog);
