var GoodsLZ = function() {
	
	var mType=2;
	
	// 打开树状图dialog
	function openGoodsLZ(id) {
	...
	// 货体流水表
	function initGoodsLS(dialog, id) {
...
//						}
	...
	function initGoodsLZ(dialog, id) {
		$.ajax({
			async : false,
			type : "get",
			url : config.getDomain()
			...
			success : function(data) {
				if(data.code=="0000"){
					...
					dialog.find('.code').html(data.data[0].cargoCode);
					dialog.find('.productName').html(data.data[0].productName);
					...
					var tree =dialog.find('.jstree').cloudTreeGrid(
								...
								render : function(item, name, index) {
									...
									if (item.goodsCurrent > 0) {
										return "<a href='javascript:void(0)'"+bold+" onclick='GoodsLZ.openGoodsLS("
												+ item.id
												+ ")' ><span class='fa fa-circle-o text-green' title='有量'></span>"
												+ item.NAME + "</a>";
									} else {
										return "<a href='javascript:void(0)'"+bold+" onclick='GoodsLZ.openGoodsLS("
												+ item.id
												+ ")'><span class='fa fa-circle-o text-red' title='无量'></span>"
												+ item.NAME + "</a>";
									}
								}
							},
							{
										...
										url : config.getDomain() + "/lading/showCaroLZ?cargoId=" + data.data[0].cargoId
							});
...
//		
//		var tree =dialog.find('.tree').treegrid(
//				{
//					columns : [
//							{
//					title : "货主名",
