...
(function($) {
    ...
    var methods = {
         ...
         * @returns {Object[]}
    	 */
		loadData : function(settings,container) {
			var columns = settings.columns;
			var head = "<tr class='heading'>";
			for(var k=0;k< columns.length;k++) {
				if(columns[k]["title"] != undefined) {
					head += "<th>"+columns[k]["title"]+"</th>";
				}
			}
			head += "</tr>";
			$(container).append(head);
			try {
				var localdata=settings.localdata;
				if(settings.localdata){
						...
						for(var i=0;i< localdata.length;i++) {
							...
							if(item["parentId"] &&  item["parentId"] != "0") {
								...
								if(hasParent) {
									body += "<tr class='treegrid-"+item["id"]+" treegrid-parent-"+item["parentId"]+"'>";
								}else {
									body += "<tr class='treegrid-"+item["id"]+"'>";
								}
							}else {
								body += "<tr class='treegrid-"+item["id"]+"'>";
							}
							...
							for(var j = 0;j < columns.length;j++) {
								try {
									if(columns[j]["render"] &&  typeof(columns[j]["render"]) == 'function' ) {
										body += "<td>"+columns[j]["render"](item,item[columns[j]["data"]],i)+"</td>";
									}else {
										body += "<td>"+item[columns[j]["data"]]+"</td>";
									}
								}catch(err) {
									body += "<td>"+item[columns[j]["data"]]+"</td>";
								}
						...
						settings.getRootNodes.apply($(container), [$(container)]).treegrid('initNode', settings);
		...
		reload : function() {
			...
			return this.each(function() {
                ...
                $(this).treegrid('loadData', settings,$(this));
        ...
        initTree: function(options) {
            ...
            return this.each(function() {
                ...
                settings.getRootNodes.apply(this, [$(this)]).treegrid('loadData', settings,$(this));
